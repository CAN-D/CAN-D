...
uint8_t BSP_SD_ReadBlocks(uint32_t* p32Data, uint64_t ReadAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
    ...
    if (SD_IO_WriteCmdLegacy(SD_CMD_SET_BLOCKLEN, BlockSize, SD_CRC_DEFAULT, SD_RESPONSE_NO_ERROR) != HAL_OK) {
        return MSD_ERROR;
    }
    ...
    while (NumberOfBlocks--) {
        ...
        if (SD_IO_WriteCmdLegacy(SD_CMD_READ_SINGLE_BLOCK, ReadAddr + offset, SD_CRC_DEFAULT, SD_RESPONSE_NO_ERROR) != HAL_OK) {
            return MSD_ERROR;
        }
...
uint8_t BSP_SD_WriteBlocks(uint32_t* p32Data, uint64_t WriteAddr, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
    ...
    while (NumberOfBlocks--) {
       ...
       Check if the SD acknowledged the write block command: R1 response (0x00: no errors) */
        if (SD_IO_WriteCmdLegacy(SD_CMD_WRITE_SINGLE_BLOCK, WriteAddr + offset, SD_CRC_DEFAULT, SD_RESPONSE_NO_ERROR) != HAL_OK) {
            return MSD_ERROR;
        }
...
uint8_t SD_GetCSDRegister(SD_CSD* Csd)
{
    ...
    if (SD_IO_WriteCmdLegacy(SD_CMD_SEND_CSD, SD_ARG_NONE, SD_CRC_DEFAULT, SD_RESPONSE_NO_ERROR) == HAL_OK) {
        if (SD_IO_WaitResponse(SD_START_DATA_SINGLE_BLOCK_READ) == HAL_OK) {
    ...
    /* NOTE: the remainder of this function was generated by STM software,
...
static uint8_t SD_GetCIDRegister(SD_CID* Cid)
{
    ...
    if (SD_IO_WriteCmdLegacy(SD_CMD_SEND_CID, SD_ARG_NONE, SD_CRC_DEFAULT, SD_RESPONSE_NO_ERROR) == HAL_OK) {
        if (SD_IO_WaitResponse(SD_START_DATA_SINGLE_BLOCK_READ) == HAL_OK) {
...
static uint8_t SD_GoIdleState(void)
{
    ...
    // The following command sequence will put the device into idle state:
    // CMD0/CMD8/CMD55/ACMD41/CMD55/ACMD41

    /* Send CMD0 (SD_CMD_GO_IDLE_STATE) to put SD in SPI mode and 
     Wait for In Idle State Response (R1 Format) equal to 0x01 */
    while (response.r1 != SD_R1_IN_IDLE_STATE) {
        counter++;
        response = SD_SendCmd(SD_CMD_GO_IDLE_STATE, SD_ARG_NONE, SD_CRC_GO_IDLE_STATE, SD_RESPONSE_R1_EXPECTED);
        SD_IO_WriteDummy();
    ...
    response = SD_SendCmd(SD_CMD_SEND_IF_COND, SD_ARG_SEND_IF_COND, SD_CRC_SEND_IF_COND, SD_RESPONSE_R7_EXPECTED);
    SD_IO_WriteDummy();
    if ((response.r1 & SD_R1_ILLEGAL_COMMAND) == SD_R1_ILLEGAL_COMMAND) {
        ...
        do {
            ...
            response = SD_SendCmd(SD_CMD_APP_CMD, SD_ARG_NONE, SD_CRC_DEFAULT, SD_RESPONSE_R1_EXPECTED);
            SD_IO_WriteDummy();

            /* Send ACMD41 (SD_CMD_SD_APP_OP_COND) to initialize SDHC or SDXC cards: R1 response (0x00: no errors) */
            response = SD_SendCmd(SD_CMD_SD_APP_OP_COND, SD_ARG_NONE, SD_CRC_DEFAULT, SD_RESPONSE_R1_EXPECTED);
            SD_IO_WriteDummy();
        } while (response.r1 == SD_R1_IN_IDLE_STATE);
        ...
        do {
            ...
            /* Send ACMD41 (SD_CMD_SD_APP_OP_COND) to initialize SDHC or SDXC cards: R1 response (0x00: no errors) */
            response = SD_SendCmd(SD_CMD_SD_APP_OP_COND, SD_ARG_APP_OP, SD_CRC_DEFAULT, SD_RESPONSE_R1_EXPECTED);
            SD_IO_WriteDummy();
        } while (response.r1 == SD_R1_IN_IDLE_STATE);
        ...
        response = SD_SendCmd(SD_CMD_READ_OCR, SD_ARG_NONE, SD_CRC_DEFAULT, SD_RESPONSE_R3_EXPECTED);
        SD_IO_WriteDummy();
...
uint8_t BSP_SD_Erase(uint32_t StartAddr, uint32_t EndAddr)
{
    ...
    if (SD_SendCmdLegacy(SD_CMD_SD_ERASE_GRP_START, StartAddr, SD_CRC_DEFAULT, SD_RESPONSE_NO_ERROR) == MSD_OK) {
        /* Send CMD33 (Erase group end) and Check if the SD acknowledged the erase command: R1 response (0x00: no errors) */
        if (SD_SendCmdLegacy(SD_CMD_SD_ERASE_GRP_END, EndAddr, SD_CRC_DEFAULT, SD_RESPONSE_NO_ERROR) == MSD_OK) {
            /* Send CMD38 (Erase) and Check if the SD acknowledged the erase command: R1 response (0x00: no errors) */
            if (SD_SendCmdLegacy(SD_CMD_ERASE, SD_ARG_NONE, SD_CRC_DEFAULT, SD_RESPONSE_NO_ERROR) == MSD_OK) {
