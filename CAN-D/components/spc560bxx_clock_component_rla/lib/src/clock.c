...
/**
 * @file    clock.c
 * @brief   SPC560Bxx clock subsystem source.
 *
 * @addtogroup CLOCK
 * @{
 */
...
void spc_clock_init(void) {
  ...
  SET_xIRCDIV(CGM.FIRC_CTL.R, (SPC5_FIRCDIV_VALUE - 1U));
  CGM.FXOSC_CTL.B.OSCDIV = (uint8_t)(SPC5_XOSCDIV_VALUE - 1U);
  CGM.SXOSC_CTL.B.OSCDIV = (uint8_t)(SPC5_SXOSCDIV_VALUE - 1U);
  SET_xIRCDIV(CGM.SIRC_CTL.R, (SPC5_SIRCDIV_VALUE - 1U));

  /* The system must be in DRUN mode on entry, if this is not the case then
     it is considered a serious anomaly.*/
  if (ME.GS.B.CURRENTMODE != (uint32_t)SPC5_RUNMODE_DRUN) {
    SPC5_CLOCK_FAILURE_HOOK();
  }
  ...
  ME.HALT0.R        = SPC5_ME_HALT_MC_BITS;     /* HALT run mode.           */
  ME.STOP0.R        = SPC5_ME_STOP_MC_BITS;     /* STOP run mode.           */
  ME.STANDBY0.R     = SPC5_ME_STANDBY_MC_BITS;  /* STANDBY run mode.        */
  if ((ME.IS.B.ICONF & 1U) == 1U) {
...
void SPCSetPeripheralClockMode(uint32_t n, uint32_t pctl) {
  ...
  mode = (uint8_t)ME.MCTL.B.TARGETMODE;
