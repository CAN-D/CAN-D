...
/**
 * @file    clock.h
 * @brief   SPC560Bxx clock subsystem header
 *
 * @addtogroup CLOCK
 * @{
 */
 ...
 */
#define SPC5_SXOSC_CLK_MAX                  40000U

/**
 * @brief   Minimum SXOSC clock frequency.
 */
#define SPC5_SXOSC_CLK_MIN                  32000U
 ...
 * @brief   Maximum FMPLLs input clock frequency.
 */
#define SPC5_FMPLLIN_MAX                    64000000U
 ...
 */
#define SPC5_FMPLL_CLK_MAX                  64000000U
/** @} */
 ...
 * @{
 */
#define SPC5_RTCC_CLKSEL_SXOSCDIV           0U
...
#define SPC5_RUNMODE_HALT                   8U
#define SPC5_RUNMODE_STOP                   10U
#define SPC5_RUNMODE_STANDBY                13U
/** @} */
...
#define SPC5_FMPLL_NDIV_VALUE               32U
#endif
...
#define SPC5_FMPLL_ODF                      SPC5_FMPLL_ODF_DIV4
#endif
...
/**
 * @brief   SXOSC clock divider value.
 */
#if !defined(SPC5_SXOSCDIV_VALUE) || defined(__DOXYGEN__)
#define SPC5_SXOSCDIV_VALUE                 1U
#endif
...
#define SPC5_API_RTC_CLK_SRC                SPC5_RTCC_CLKSEL_SXOSCDIV
#endif
...
/* Check on the SXOSC divider.*/
#if (SPC5_SXOSCDIV_VALUE < 1U) || (SPC5_SXOSCDIV_VALUE > 32U)
#error "invalid SPC5_SXOSCDIV_VALUE value specified"
#endif
...
/**
 * @brief   Set FIRCDIV and SIRCDIV
 * @note    FIRCDIV is a field of the register FIRC_CTL. SIRDIV is a field of
 *          the register SIRC_CTL. Both registers also contain a field called
 *          TRIM. In order to avoid any unintentionally modification to the
 *          value of the field TRIM during the update of the value of the 
 *          FIRCDIV or of the SIRDIV, it is mandatory to write only the upper
 *          16 bits of the registers FIRC_CTL and SIRC_CTL.
 *
 * @param[in] reg       FIRC_CTL/SIRC_CTL register address
 * @param[in] val       FIRCDIV/SIRCDIV value
 *
 * @noapi
 */
#define SET_xIRCDIV(reg, val) {                             \
  uint32_t regaddr;                                         \
  volatile uint16_t *reghp;                                 \
  uint16_t reghval;                                         \
  regaddr = (uint32_t)(&(reg));                             \
  reghp = (volatile uint16_t *)(regaddr);                   \
  reghp++;                                                  \
  reghval = (uint16_t)(0x01F00U & ((uint16_t)(val) << 8U)); \
  *reghp = (*reghp & 0xE0FFU) | reghval;                    \
}
